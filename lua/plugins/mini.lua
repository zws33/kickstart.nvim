return {
  ...
  config = function()
    ...
    end, { desc = "[E]xplore at current file" })

    vim.keymap.set("n", "<leader>eE", function()
      require("mini.files").open(vim.uv.cwd(), true)
    end, { desc = "[E]xplore at cwd" })

    vim.keymap.set("n", "<leader>er", function()
      require("mini.files").reveal(vim.api.nvim_buf_get_name(0), true)
    end, { desc = "[E]xplore [R]eveal current file" })
    ...
    end, { desc = "[W]orkspace [S]ave session" })

    vim.keymap.set("n", "<leader>wS", function()
      require("mini.sessions").select()
    end, { desc = "[W]orkspace [S]elect session" })
    ...
    require("mini.clue").setup({
      triggers = {
        ...
        -- mini.basics option toggle prefix
        { mode = "n", keys = "\\" },
      },
    ...
    vim.api.nvim_create_autocmd("BufEnter", {
      callback = function(ev)
        ...
        if vim.g.MiniClue ~= nil and MiniClue.ensure_buf_triggers ~= nil then
